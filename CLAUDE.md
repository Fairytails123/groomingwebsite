...
- `src/data/pricing.json` — ALL prices (~105 breed rows + add-ons + extras). Generated by
  ...
  ⚠️ **Not all of it is harvested.** The whole `subscription` block — including
  `subscription.optionalAddOn` (the £10/month teeth cleaning, live 2026-09-04) — is a **hardcoded
  literal inside `scripts/extract-prices.mjs`**. It is therefore absent from
  `grooming-image-archive/`: **that is correct, not foreign data.** Regeneration is byte-identical
  (measured 2026-09-04); after any run, the `pricing.json` diff should be your change ONLY.
  ⚠️ `subscription.optionalAddOn` and the `addOns` "Teeth cleaning" row are the SAME treatment
  sold two ways (monthly vs per visit), both £10. **Never merge them and never RENAME either** —
  `services/teeth-cleaning.astro` finds the `addOns` row by exact name. Changing one £10 requires
  an explicit owner decision about the other; they can silently diverge.
...
- ⚠️ **TWO live Stripe URLs, not interchangeable**, both in `business.subscription` on account
  `acct_1MiHBhKxaWtXebCj`: `stripeUrl` → `buy.stripe.com/…` (payment link, **sign-up only**) and
  `portalUrl` → `billing.stripe.com/p/login/…` (Billing Customer Portal, **existing subscribers
  only**, live 2026-09-04). 🔴 **They end in the SAME token `8x27sM5K57BR1IL94W9MY00`** — a Stripe
  coincidence, NOT duplication. Never "DRY" them into one constant, never treat one as a typo of
  the other. A portal link from a different Stripe account tells every subscriber "no subscriptions
  found". Both act on real money: never complete a test checkout, and never drive the portal
  through to a change — it edits a paying customer's live subscription.
- ⚠️ Always show "£25/month per dog, 2-month minimum term" next to the sign-up CTA, and the
  optional "£10 a month" teeth-cleaning line. Full rules: § "Stripe subscriptions & the billing
  portal" under Integrations.
...
| `verify-stage3` | Services-cluster facts: 105 rows render, 0 hidden at t=0, spot-check vs the rendered table, and no banned pick-up/policy wording anywhere. **Since 2026-09-04 it also guards the subscription**: both Stripe URLs by FULL href on the three bands + the footer, the footer Explore grid capacity, the £25/£10/2-month literals, the pay-per-visit `addOns` row resolving by exact name, and the T&C subscription clause (heading, non-refund sentence, phone-only cancellation). Mutation-tested 10/10, 0 false positives. Its fuzzy "contradicts the Stripe config" copy scan is deliberately **WARN-only** — this script also gates the reviews rotator's unattended Monday commit, where a false failure would silently freeze the carousel. |
...
change.
⚠️ **Same blind spot, live example.** The footer's Explore list is `grid-flow-col grid-rows-8`, so
it holds **2 × 8 = 16 links and currently has 15**. At `grid-rows-7` the 15th silently spilled into
a THIRD sub-column and broke every desktop page. `mobile-check` renders 390px only, where that
footer is single-column, so it structurally CANNOT see this. `verify-stage3` now asserts
`links <= rows × 2` — adding a 17th link means bumping the row count too. And never "fix" a footer
link's tap size with `min-h-11`: one taller grid row breaks the two-column rhythm, and
`mobile-check` exempts `<a>` inside `li`, so no gate would catch that either.
...
1. Copy fact-checked against `grooming-image-archive/<slug>/copy.md` (prices/phones/hours verbatim).
   ⚠️ **Two owner-instructed additions are deliberately NOT in the harvest and must survive this
   check**: the "Dog grooming subscription" section in `terms-and-conditions.astro` (the harvest
   has 18 sections and no subscription one) and `subscription.optionalAddOn` in `pricing.json`.
   Do not delete them as foreign copy — `verify-stage3` asserts both.
...
6. HANDOVER.md updated; push. ⚠️ **A push to `main` IS the deploy, and the deploy is LIVE to
   ...
   `npm run verify-urls -- --live` (19/19) and eyeball `https://fairytailsdoggrooming.co.uk/`
   at a phone viewport.
...
- **Grooming-only analytics:** GTM `GTM-TZWLLT4H`; GA4 `G-TVNY7185K3`. Consent Mode v2 defaults
  ...
  **Never mix this site with `www.thefairytails.co.uk` in any third-party account, property,
  ...
  wholesale.
- **Stripe subscriptions & the billing portal** (live 2026-09-04) → the Stripe-hosted **Billing
  Customer Portal**, linked from `business.subscription.portalUrl`; config
  `bpc_1UBwdLKxaWtXebCj4qyueId5` on account `acct_1MiHBhKxaWtXebCj`.
  **There is NO login on this site and none is wanted.** The customer types the email they
  subscribed with and **Stripe** emails them a one-time link; a non-customer never receives one.
  **Stripe is the eligibility gate — never build one here**, and never call the portal an
  "account", "login" or "member area" in copy.
  ⚠️ **Never append `?prefilled_email=` to that URL from the site.** Stripe supports it, but it
  would put a customer's email address into a URL and into analytics — on a public repo.
  ⚠️ **The portal's behaviour lives in the Stripe Dashboard, NOT in this repo, and no gate can see
  it.** Every config note here is a DATED SNAPSHOT; the first one was false within hours.
  **Re-read the Dashboard before writing any copy about what customers can do.** As of
  2026-09-04, live-verified: self-service **CANCEL is OFF** (owner ruling — the only thing keeping
  the 2-month minimum term enforceable, so cancelling is a phone call), **quantity updates are ON**
  across `Dog Grooming Subscription` (£25/mo) and `Teeth Cleaning Optional Sub` (£10/mo), plan
  switching OFF, proration "no charges or credits", invoice history + payment methods + billing
  details ON. If cancel is ever flipped back on, the site copy becomes a lie and `verify-stage3`
  will keep enforcing it — **the gate guards the copy, not the config.**
  ⚠️ Where several customer records share one email, Stripe opens only "the most recently created
  customer that has both that email and an active subscription", so a duplicate record hides a
  second live subscription. One such case is open — see HANDOVER 2026-09-04.
